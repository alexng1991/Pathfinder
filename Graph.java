...
public int dijkstra( Vertex s ) {
  ...
  s.cost = 0;
  s.parent = null;
  PriorityQueue<Vertex> fringe = new PriorityQueue<Vertex>(20,
    ...
    if ( ! v.visited ) {
        v.visited = true;
          for ( Edge e : v.edges ) {
            int newcost = v.cost + e.cost;
              if ( newcost < e.target.cost ) {
                int newest = h.fn(e.target, goal);
                e.target.estimate = newest;
                e.target.cost = newcost;
                e.target.parent = v;
                fringe.add(e.target);
           }
...
public static final Heuristic randombs = new Heuristic() 
{
  public Integer fn(Vertex from, Vertex to)
  {
    return (Integer)((int)rng.nextDouble() * from.distanceTo(to));
  }
};
...
public Cons pathto(String city) {
  Cons ans = null;
  Vertex v = vertex(city);
  while (v != null) 
  {
    ans = Cons.cons(v.str(), ans);
    v = v.parent();
  }
  return ans;
}
